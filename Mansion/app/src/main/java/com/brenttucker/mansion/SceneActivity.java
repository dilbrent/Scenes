package com.brenttucker.mansion;
...
public class SceneActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    ModelRenderable.builder()
        .setSource(this, Uri.parse("mansion.sfb"))
        .build()
        .thenAccept(renderable -> stageRenderable = renderable)
        .exceptionally(
            throwable -> {
              Toast toast =
                  Toast.makeText(this, "Unable to load mansion", Toast.LENGTH_LONG);
      ...
      arFragment.setOnTapArPlaneListener(
        (HitResult hitResult, Plane plane, MotionEvent motionEvent) -> {
          ...
          node.setRenderable(stageRenderable);
          node.getScaleController().setMaxScale(5.0f);
          node.getScaleController().setMinScale(0.2f);
          node.getScaleController().setElasticity(.2f);
          node.getScaleController().setSensitivity(.04f);
          node.select();
        });
  }
